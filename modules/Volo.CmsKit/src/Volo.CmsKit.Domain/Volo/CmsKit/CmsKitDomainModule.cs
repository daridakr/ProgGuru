...
public class CmsKitDomainModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        if (GlobalFeatureManager.Instance.IsEnabled<TagsFeature>())
        {
            ...
            {
                options.EntityTypes.Add(new TagEntityTypeDefiniton("Project"));
            });

            // COMMENTS
            Configure<CmsKitCommentOptions>(options =>
            {
                options.EntityTypes.Add(new CommentEntityTypeDefinition("Project"));
            });
            Configure<CmsKitCommentOptions>(options =>
            {
                options.EntityTypes.Add(new CommentEntityTypeDefinition("User"));
            });

            // RATINGS
            Configure<CmsKitRatingOptions>(options =>
            {
                options.EntityTypes.Add(new RatingEntityTypeDefinition("Project"));
            });
            Configure<CmsKitRatingOptions>(options =>
            {
                options.EntityTypes.Add(new RatingEntityTypeDefinition("User"));
            });
