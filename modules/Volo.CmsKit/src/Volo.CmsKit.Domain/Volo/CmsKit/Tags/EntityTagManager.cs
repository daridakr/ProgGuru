...
public class EntityTagManager : DomainService
{
    ...
    public async Task RemoveEntityTagsAsync(string entityType, string entityId)
    {
        var existingTags =
          await TagRepository.GetAllRelatedTagsAsync(entityType, entityId);
        ...
        {
            await RemoveTagFromEntityAsync(existingTag.Id, entityType, entityId, CurrentTenant?.Id);
        }
    }
}
